...
export class UIHandlers {
  ...
  public static async generateAndSelectBranch(): Promise<void> {
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        progress.report({
          increment: 0,
          message: "Generating branch name suggestions...",
        });
        ...
        try {
          ...
          const suggestions = await aiService.generateBranchNames(
            context,
            undefined,
            aiConfig.suggestionCount,
          );

          progress.report({
            increment: 100,
            message: "Branch names generated!",
          });
  ...
  public static async generateBranchFromCommitMessage(): Promise<void> {
    ...
    // Get the current commit message from the SCM input box
    let defaultMessage = "";
    ...
    const commitMessage = await vscode.window.showInputBox({
      ...
      placeHolder:
        "Add user authentication feature. This commit adds login form, OAuth integration, and session management.",
   ...
   */
  private static async showBranchSelection(
    suggestions: string[],
  ): Promise<void> {
  ...
  private static handleAIServiceError(error: AIServiceError): void {
    ...
    switch (error.code) {
      case "CONFIG_ERROR":
        vscode.window
          .showWarningMessage(message, "Open Settings", "Cancel")
          .then((selection) => {
            if (selection === "Open Settings") {
              this.openSettings();
            }
      ...
      default:
        vscode.window.showErrorMessage(message, "Cancel");
        break;
    }
  }
  ...
  private static handleGitOperationError(error: GitOperationError): void {
    ...
    switch (error.code) {
      case "BRANCH_EXISTS":
        vscode.window.showWarningMessage(message, "Cancel");
        break;
